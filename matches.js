...
function startCountdownEngine() {
  ...
  countdownEngineInterval = setInterval(() => {
    allTournaments.forEach(t => {
      const miniEl = document.getElementById(`mini-countdown-${t.id}`);
      ...
      if (!targetDate) {
        if (miniEl) miniEl.innerText = "00d 00h 00m 00s";
        return;
      }

      const diff = targetDate.getTime() - Date.now();
      if (diff <= 0) {
        if (miniEl) miniEl.innerText = "STARTED / LIVE";
        if (fullEl) {
          fullEl.innerHTML = `
            <div class="text-xs font-bold text-green-400 font-grotesk flex items-center gap-1.5 bg-green-950/20 border border-green-500/30 px-3 py-1.5 rounded-xl">
      ...
      if (miniEl) {
        miniEl.innerText = `${dStr}d ${hStr}h ${mStr}m ${sStr}s`;
      }

      if (fullEl) {
        fullEl.innerHTML = `
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            <span class="block text-[11px] font-bold text-white leading-none">${dStr}</span>
            <span class="text-[6px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">DAYS</span>
          </div>
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            <span class="block text-[11px] font-bold text-white leading-none">${hStr}</span>
            <span class="text-[6px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">HRS</span>
          </div>
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            <span class="block text-[11px] font-bold text-white leading-none">${mStr}</span>
            <span class="text-[6px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">MINS</span>
          </div>
          <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
            <span class="block text-[11px] font-bold text-white leading-none">${sStr}</span>
            <span class="text-[6px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">SECS</span>
          </div>
        `;
      }
    });
...
export function updateMatchDetailsModalContent(tournamentId) {
  ...
  // Double buttons footer layout integration inside Details Popup modal
...
export function renderMatches() {
  ...
  filtered.forEach(t => {
    ...
    card.innerHTML = `
      ...
      <div class="relative h-[160px] w-full overflow-hidden shrink-0 bg-[#0c071a]">
        <img
          src="${bannerSrc}"
          alt="${t.title}"
          loading="lazy"
          class="absolute inset-0 w-full h-full object-cover z-0 transition-transform duration-500 hover:scale-105"
          style="opacity: 0.8 !important; filter: none !important;"
        ...
        <div class="absolute top-4 left-4 right-4 flex justify-between items-start z-10">
          <div class="flex items-center gap-2">
            <span class="px-2.5 py-1 text-[10px] font-black uppercase tracking-wider font-rajdhani rounded-md text-white" style="background:${modeBg[mode] || modeBg.Solo};">
              ${mode}
            </span>
            <span class="text-white font-rajdhani font-black text-base sm:text-lg tracking-wider uppercase drop-shadow-md truncate max-w-[120px] sm:max-w-[200px]">${t.title}</span>
          </div>
          <!-- Right Status Badge -->
        ...
        <div class="absolute bottom-4 left-4 z-10 space-y-0.5">
          <div class="text-[#c084fc] font-grotesk font-extrabold text-xs tracking-wider uppercase">${mapName} | TPP</div>
          <div class="flex items-center gap-2.5 text-[10px] text-slate-300 font-semibold font-grotesk uppercase">
            <span class="flex items-center gap-1"><i class="fa-regular fa-calendar text-gold text-[9px]"></i> ${t.date || '—'}</span>
            <span class="flex items-center gap-1"><i class="fa-regular fa-clock text-gold text-[9px]"></i> ${t.time || '—'}</span>
          </div>
        </div>

        <!-- Right Bottom Starts In Overlay Countdown Badge -->
        <div class="absolute bottom-4 right-4 z-10 bg-black/75 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-gold/15 text-right font-grotesk min-w-[110px]">
          <div class="text-[8px] text-slate-400 font-extrabold uppercase tracking-wider">STARTS IN</div>
          <div class="text-[10px] font-mono font-bold text-yellow-400 mt-0.5" id="mini-countdown-${t.id}">00d 00h 00m 00s</div>
        </div>
      </div>

      <!-- MAIN CARD CONTENT BODY -->
      <div class="p-4 space-y-4 bg-[#05020c]/95 border-t border-[#8b5cf6]/10">

        <!-- FOUR EQUAL PRIZE BOXES GRID -->
        <div class="grid grid-cols-4 gap-2 text-center font-grotesk">
          <div class="bg-[#120726]/40 border border-gold/10 hover:border-gold/30 p-2 rounded-xl transition-all shadow-[inset_0_0_10px_rgba(212,175,55,0.02)]">
            <span class="block text-[8px] uppercase tracking-wider text-slate-500 font-extrabold leading-none">Prize Pool</span>
            <strong class="block text-sm text-yellow-400 font-rajdhani font-black mt-1">₹${t.prizePool || 0}</strong>
          </div>
          <div class="bg-[#120726]/40 border border-[#8b5cf6]/10 hover:border-[#8b5cf6]/30 p-2 rounded-xl transition-all shadow-[inset_0_0_10px_rgba(139,92,246,0.02)]">
            <span class="block text-[8px] uppercase tracking-wider text-slate-500 font-extrabold leading-none">Per Kill</span>
            <strong class="block text-sm text-purple-400 font-rajdhani font-black mt-1">₹${t.perKill || 0}</strong>
          </div>
          <div class="bg-[#120726]/40 border border-emerald-500/10 hover:border-emerald-500/30 p-2 rounded-xl transition-all shadow-[inset_0_0_10px_rgba(16,185,129,0.02)]">
            <span class="block text-[8px] uppercase tracking-wider text-slate-500 font-extrabold leading-none">Entry Fee</span>
            <strong class="block text-sm text-emerald-400 font-rajdhani font-black mt-1">₹${t.entryFee || 0}</strong>
          </div>
          <div class="bg-[#120726]/40 border border-blue-500/10 hover:border-blue-500/30 p-2 rounded-xl transition-all shadow-[inset_0_0_10px_rgba(59,130,246,0.02)] cursor-pointer winner-prize-trigger">
            <span class="block text-[8px] uppercase tracking-wider text-slate-500 font-extrabold leading-none">Winner Prize</span>
            <strong class="block text-sm text-blue-400 font-rajdhani font-black mt-1 truncate">₹${winnerPrize} <i class="fa-solid fa-circle-info text-[9px]"></i></strong>
          </div>
        </div>

        <!-- PLAYER ROSTER ALLOCATIONS SECTION -->
        <div class="space-y-2 font-grotesk">
          <div class="flex justify-between items-center text-[10px] tracking-wider font-bold">
            <div class="flex items-center gap-1.5 text-yellow-500">
              <i class="fa-solid fa-users"></i>
              <span>${joined} / ${max} Joined</span>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-[#c084fc]">Remaining Slots: ${remaining}</span>
              <!-- Availability Badge -->
              <span class="flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${
                remaining <= 0 ? 'bg-red-500/10 border border-red-500/40 text-red-400' :
                remaining <= 10 ? 'bg-orange-500/10 border border-orange-500/40 text-orange-400 animate-pulse' :
                'bg-emerald-500/10 border border-emerald-500/40 text-emerald-400'
              }">
                <span class="w-1.5 h-1.5 rounded-full ${
                  remaining <= 0 ? 'bg-red-500' :
                  remaining <= 10 ? 'bg-orange-500' :
                  'bg-emerald-500'
                }"></span>
                ${remaining <= 0 ? 'FULL' : remaining <= 10 ? 'ALMOST FULL' : 'AVAILABLE'}
              </span>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <div class="progress-bar-wrap h-1.5 flex-1">
            ...
            <span class="text-[10px] text-slate-400 font-bold font-mono shrink-0">${progress}%</span>
          </div>
        </div>

        <!-- COUNTDOWN BOXES SECTION -->
        <div class="bg-black/50 border border-[#8b5cf6]/20 rounded-2xl p-3 flex items-center justify-between gap-3 text-slate-300">
          <div class="flex flex-col gap-0.5">
            <div class="flex items-center gap-1 text-[9px] font-extrabold text-[#c084fc] uppercase tracking-wider font-grotesk">
              <i class="fa-regular fa-clock"></i>
              <span>MATCH STARTS IN</span>
            </div>
            <!-- Countdown grid matching reference -->
            <div class="flex items-center gap-1 mt-1 font-mono text-center" id="full-countdown-${t.id}">
              <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
                <span class="block text-[11px] font-bold text-white leading-none">00</span>
                <span class="text-[6px] text-slate-500 uppercase font-extrabold tracking-tighter block mt-0.5">DAYS</span>
              </div>
              <div class="bg-black/60 border border-purple-500/10 rounded px-1.5 py-1 min-w-[32px]">
                <span class="block text-[11px] font-bold text-white leading-none">00</span>
          ...
          <div class="text-right space-y-0.5 border-l border-[#8b5cf6]/15 pl-3">
            <span class="text-[8px] text-slate-500 font-extrabold uppercase tracking-wider flex items-center gap-1 justify-end font-grotesk">
              <i class="fa-regular fa-calendar-check text-gold"></i> Starts At
            </span>
            <strong class="block text-[10px] text-white font-grotesk font-extrabold leading-tight">${t.date || '—'}</strong>
            <span class="block text-[10px] text-yellow-500 font-mono leading-none">${t.time || '—'}</span>
          </div>
        </div>

        <!-- ROOM CREDENTIALS PANEL -->
        <div class="bg-[#120924]/60 border border-purple-500/15 rounded-xl px-2.5 py-1.5 flex items-center justify-between min-h-[36px] font-grotesk">
          <span class="text-slate-400 font-bold tracking-wider text-[9px] uppercase">ROOM DETAILS:</span>
          <div class="flex items-center gap-2">
            ${
              isJoined 
              ? `
                <span class="flex items-center gap-1.5 bg-purple-950/60 px-2 py-0.5 rounded border border-purple-500/20 text-[10px] font-mono">
                  ID: <strong class="text-white">${showId ? t.roomId : "HIDDEN 🔒"}</strong>
                  ${showId ? `<i class="fa-regular fa-copy cursor-pointer text-gold hover:text-white transition-colors" onclick="event.stopPropagation(); window.copyValue('${t.roomId}', 'Room ID')"></i>` : ''}
                </span>
                <span class="flex items-center gap-1.5 bg-purple-950/60 px-2 py-0.5 rounded border border-purple-500/20 text-[10px] font-mono">
                  PW: <strong class="text-white">${showPw ? (t.roomPass || t.roomPassword || "No Password") : "HIDDEN 🔒"}</strong>
                  ${showPw ? `<i class="fa-regular fa-copy cursor-pointer text-gold hover:text-white transition-colors" onclick="event.stopPropagation(); window.copyValue('${t.roomPass || t.roomPassword || ''}', 'Password')"></i>` : ''}
                </span>
              `
              : `
                <span class="flex items-center gap-1 bg-black/40 px-2 py-0.5 rounded border border-purple-500/5 text-[10px] text-slate-500 font-mono">ID: HIDDEN 🔒</span>
                <span class="flex items-center gap-1 bg-black/40 px-2 py-0.5 rounded border border-purple-500/5 text-[10px] text-slate-500 font-mono">PW: HIDDEN 🔒</span>
              `
            }
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 pt-1">
          <button class="py-2.5 px-3 bg-[#120726]/80 hover:bg-[#1a0a36]/90 border border-[#8b5cf6]/40 hover:border-[#a78bfa]/60 text-white rounded-xl font-rajdhani font-black text-sm uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all shadow-[0_0_15px_rgba(139,92,246,0.1)] hover:shadow-[0_0_20px_rgba(139,92,246,0.25)] view-details-btn">
            VIEW DETAILS <i class="fa-solid fa-arrow-right text-[10px]"></i>
          </button>
          
          <button class="py-2.5 px-3 bg-gradient-to-r from-gold via-gold-light to-gold-dark text-black rounded-xl font-rajdhani font-black text-sm uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all glow-gold-btn join-match-btn">
            <i class="fa-solid fa-gamepad"></i> JOIN MATCH
          </button>
        </div>
    ...
    // Programmatically attach card details modal click trigger
    ...
    // Explicit Action button routes targeting individual modals directly
...
window.switchView = function(viewId, param = null) {
  ...
  requestAnimationFrame(() => {
    ...
    if (viewId === 'matches') {
      ...
      // Clear search input on switching mode views to prevent overlapping queries
      const searchInput = document.getElementById('match-search-input');
...
export function initMatchesSync() {
  ...
  // Bind keyup and input operations targeting search elements once snapshot is initialized
  const searchInput = document.getElementById('match-search-input');
  if (searchInput) {
